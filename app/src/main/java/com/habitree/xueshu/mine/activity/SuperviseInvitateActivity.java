 ...
 * @author wuxq
 */
public class SuperviseInvitateActivity extends BaseActionBarActivity implements FriendsListView, SendMsgView, OnFriendSelectListener {
    ...
    protected void initData() {
        setTitle("邀请好友");
        habit_id = getIntent().getIntExtra(Constant.HABIT_ID, -1);
        showLoadingDialog();
    ...
    public void onFriendSelect(Friend friend) {
        if (friend != null) {
            ...
            String title = "监督邀请";
            String message = myName + "向你发来一条习惯监督的请求";
            showLoadingDialog();
            MessageManager.getManager().sendMessage(this, friend.mem_id, title, message,
                    SendMessageType.HABIT_INVITATION, habit_id, 0, this);
        }
    }
