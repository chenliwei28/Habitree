...
public class HabitPresenter extends BasePresenter {
     ...
     * @param list_type
     */
    public void getHabitCheckList(int page, int offset, int type, int list_type, final HabitView.HabitSuperviseListView view) {
        String timestamp = String.valueOf(TimeUtil.getCurrentMillis());
        HttpManager.getManager().getService().getHabitCheckList(timestamp, CommUtil.getSign(Constant.CHECK_HABIT_LIST, timestamp),
                UserManager.getManager().getUser().user_token, page, offset, type, list_type)
                .enqueue(new Callback<CheckListResponse>() {
                    ...
                    public void onResponse(Call<CheckListResponse> call, Response<CheckListResponse> response) {
                        if (response.body() != null) {
                            if (CommUtil.isSuccess(mContext, response.body().status)) {
                                view.onGetSuperviseListSuccess(response.body().data);
                            } else {
                                view.onGetSuperviseListFailed(CommUtil.unicode2Chinese(response.body().info));
                            }
                        }
                    }
                    ...
                    public void onFailure(Call<CheckListResponse> call, Throwable t) {
                        view.onGetSuperviseListFailed(mContext.getString(R.string.network_error));
                    }
                });
    }
}
